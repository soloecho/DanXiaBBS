...
class logging_ctl {
	...
	function on_login() {
		...
		if(!submitcheck('loginsubmit', 1, $seccodestatus)) {
			...
			if($result['status'] > 0) {
				...
				//Add cookie to uniform login with DanXia
				dsetcookie('dx_username', $result['member']['username'], 0, 0);
				dsetcookie('dx_usertype', $result['member']['adminid'] ? 'admin' : 'passer', 0, 0);
				dsetcookie('dx_formhash', $_G['formhash'], 0, 0);
	...
	function on_logout() {
		...
		clearcookies();
		//Delete cookie to uniform logout with DanXia
		dsetcookie('dx_username', '', 0, 0);
		dsetcookie('dx_usertype', '', 0, 0);
		dsetcookie('dx_formhash', '', 0, 0);
